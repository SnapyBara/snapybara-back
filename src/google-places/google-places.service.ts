...
export class GooglePlacesService {
  ...
  private filterRelevantPlacesWithNature(places: any[]): any[] {
    ...
    return places.filter((place) => {
      ...
      if (natureNameKeywords.some((keyword) => nameLower.includes(keyword))) {
        this.logger.debug(
          `Including place "${place.displayName?.text || place.name}" based on name keywords`,
        );
        return true;
      }
      ...
      async () => {
        try {
          ...
          const headers = {
            ...
            'X-Goog-FieldMask':
              'places.name,places.id,places.displayName,places.location,places.formattedAddress,places.types,places.rating,places.userRatingCount,places.photos,places.businessStatus,places.priceLevel',
          };
          ...
          return filteredPlaces.map((place: PlaceV1) =>
            this.convertV1ToLegacyFormat(place),
          );
        } catch (error) {
  ...
  async textSearch(
    ...
    if (!params.query || params.query.length < 3) {
      this.logger.debug(
        'Query too short for text search, returning empty results',
      );
      return [];
    }
    ...
    // Générer la clé de cache
    const cacheKey =
      this.cacheService.generateGooglePlacesTextSearchKey(params);
      ...
      async () => {
        try {
          ...
          const headers = {
            'X-Goog-Api-Key': this.apiKey!,
            'X-Goog-FieldMask':
              'name,id,displayName,location,formattedAddress,types,rating,userRatingCount,photos,currentOpeningHours,nationalPhoneNumber,websiteUri,priceLevel,businessStatus',
          };

          this.logger.debug(`Making new Places API details request to: ${url}`);
  ...
  convertToPointOfInterest(
    ...
    const categoriesFromTypes = this.mapGoogleTypesToOurCategories(
      googlePlace.types,
    );
    ...
    // Log pour debug
    if (
      googlePlace.types.some((t) =>
        ['tourist_attraction', 'point_of_interest', 'establishment'].includes(
          t,
        ),
      )
    ) {
      this.logger.debug(
        `Converting place "${googlePlace.name}" - types: ${googlePlace.types.join(', ')}, initial category: ${categoriesFromTypes[0] || 'other'}, refined category: ${refinedCategory}`,
      );
    }
    ...
    return {
      ...
      metadata: {
        ...
        photos: googlePlace.photos?.slice(0, 3)?.map((photo) => ({
          // Limiter à 3 photos
  ...
  private refineCategoryByName(
    ...
    const mountainExclusions = [
      'montcalm',
      'montpellier',
      'montreal',
      'montgomery',
    ]; // Noms de villes/quartiers à exclure

    if (mountainKeywords.some((keyword) => nameLower.includes(keyword))) {
      // Vérifier que ce n'est pas un nom de ville ou de quartier
      if (
        !mountainExclusions.some((exclusion) => nameLower.includes(exclusion))
      ) {
    ...
    if (
      ...
      if (natureKeywords.some((keyword) => nameLower.includes(keyword))) {
        this.logger.debug(
          `Categorizing "${name}" as landscape based on nature keywords`,
        );
        return 'landscape';
      }
    }
    ...
    this.logger.debug(
      `Keeping default category "${defaultCategory}" for "${name}" (types: ${types.join(', ')})`,
    );
    ...
    return this.cacheService.getOrSet(
      ...
      async () => {
        try {
          ...
          this.logger.debug(
            `Autocomplete: ${predictions.length} predictions found before filtering`,
          );
      ...
      */
          ...
          this.logger.debug(
            `Autocomplete: ${filteredPredictions.length} predictions after filtering`,
          );
          filteredPredictions.forEach((pred, index) => {
            this.logger.debug(
              `  ${index + 1}. ${pred.mainText} - ${pred.secondaryText} (types: ${pred.types.join(', ')})`,
            );
          });
  ...
  private mapGoogleTypesToOurCategories(googleTypes: string[]): string[] {
    ...
    const mappedCategories = priorityOrder.filter((cat) =>
