...
export class AuthController {
  ...
  async googleAuth(@Body() googleAuthDto: GoogleAuthDto): Promise<AuthResponseDto> {
    ...
    console.log('Google auth request received:', { idToken: googleAuthDto.idToken?.substring(0, 20) + '...' });
    ...
    try {
      ...
      const { data: existingUser } = await this.supabaseAdmin.auth.admin.listUsers();
      ...
      const customToken = await this.authService.generateSupabaseCompatibleToken(user);
